...
for(i in 1:no.levels){
	...
	print(paste('running normalization, i = ',i))
	if(is.na(match('order',colnames(data01)))){	
		select <- levels(data01[,match(TimeSeries,Names)])[i]}else{
		temp.data <- subset(data01,data01$order==i)
		select <- temp.data[,match(TimeSeries,Names)][1]
		}
	dat.col <- match(TimeSeries,Names)	
